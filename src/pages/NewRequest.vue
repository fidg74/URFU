<template>
  <b-container class="newRequest">
    <b-button
      variant="primary"
      class="btn_flat mb-sm-3 newRequest__top"
      to="/requests"
    >
      <div class="iconBack">
        <svg
        ...
        >
          <path
            d="M4.16424 1.33576L1.17145 4.32855M1.17145 4.32855L4.16424 7.32134M1.17145 4.32855H10.8285"
            stroke="#467BE3"
            stroke-width="1.5"
    ...
    <div class="h1__description">
      Расскажите о&nbsp;задаче. Это поможет преподавателям и&nbsp;студентам
      сделать выбор проекта.
      <div v-if="this.semesterActual">
        Реализация проекта может быть начата с&nbsp;<b
          >{{ this.formatDate(this.semesterActual.start_date) }}&nbsp;г.</b
        >
      </div>
    </div>
    ...
    <b-row class="mt-sm-4">
      <b-col>
        <div v-pin-aside class="project-status__wrap">
          ...
          <div class="project-status">
            ...
            <div class="project-status__title">
      ...
      <b-col cols="12">
        <b-card class="card_content mt-0">
          ...
          <b-form-group label="Название проекта" class="mt-sm-4">
            <div class="form-actions">
              <i
                :class="{ 'form-help_show': request.name }"
                class="form-help"
                v-b-tooltip.hover
                show
                title="Напишите название проекта"
              />
            </div>
            <b-form-input
              ...
              :class="{
                'form-control_error':
                  $v.request.$dirty && $v.request.name.$invalid,
              }"
              type="text"
              autocomplete="off"
            ...
            <div v-if="$v.request.$dirty">
              <div class="form-error" v-if="!$v.request.name.required">
                Поле не заполнено
              </div>
            </div>
          </b-form-group>
          <b-form-group label="Цель">
            <div class="form-actions">
              <i
                :class="{ 'form-help_show': request.goal }"
                class="form-help"
                v-b-tooltip.hover.html
                title="Опишите, что должно быть достигнуто в&nbsp;результате выполнения проекта. Ставьте измеримые и&nbsp;выполнимые цели. Ответьте на&nbsp;вопрос, для чего создается результат проекта.<br /><br />Например, цель разработки веб-сайта: посетители сайта будут уверены, что существует угроза глобального потепления."
              />
            </div>
            <Editor
            ...
            />
            <div v-if="$v.request.$dirty">
              <div class="form-error" v-if="!$v.request.goal.requiredHTML">
                Поле не заполнено
              </div>
            </div>
          </b-form-group>
          <b-form-group label="Результат (продукт)">
            <div class="form-actions">
              <i
                :class="{ 'form-help_show': request.result }"
                class="form-help"
                v-b-tooltip.hover
                show
                title="Опишите, что должна создать команда проекта."
              />
            </div>
            <Editor
            ...
            />
            <div v-if="$v.request.$dirty">
              <div class="form-error" v-if="!$v.request.result.requiredHTML">
                Поле не заполнено
              </div>
            </div>
          </b-form-group>
          <b-form-group label="Критерии приемки результата / продукта">
            <div class="form-actions">
              <i
                :class="{ 'form-help_show': request.criteria }"
                class="form-help"
                v-b-tooltip.hover
                title="Указывайте измеримые критерии, которые должны быть достигнуты для того, чтобы проект мог считаться успешным."
              />
            </div>
            <Editor
            ...
            />
            <div v-if="$v.request.$dirty">
              <div class="form-error" v-if="!$v.request.criteria.requiredHTML">
                Поле не заполнено
              </div>
            </div>
          </b-form-group>
          <b-form-group label="Описание проекта">
            <div class="form-actions">
              <i
                :class="{ 'form-help_show': request.description }"
                class="form-help"
                v-b-tooltip.hover
                title="Опишите особенности проекта, технологии и способы реализации. Рекомендуем указать компетенции, которые понадобятся участникам проекта."
              />
            </div>
            <Editor
            ...
            <div v-if="$v.request.$dirty">
              <div
                class="form-error"
                v-if="!$v.request.description.requiredHTML"
              >
                Поле не заполнено
              </div>
              <div class="form-error" v-if="!$v.request.description.minLength">
        ...
        <b-card class="card_content card_content_program-select">
          <ProgramFilterSelect
        ...
        <b-card class="card_content">
          <h2>Приложения</h2>
          <div class="text-subtitle">
            Вы&nbsp;можете приложить файлы с&nbsp;дополнительной информацией
            о&nbsp;проекте.<br />Например, техническое задание или презентацию.
          </div>

          <FileUpload
            :address="
              learning_src + 'request/' + $route.params.id + '/file_upload/'
            "
          ...
          <FileDraftUpload
            formslug="new-request"
            @uploaded="fileUploaded"
            v-else
          />

          <div
            v-if="request.req_files && request.req_files.length"
            class="mt-4"
          >
            <FileDownload
              remove
              ...
              :file="file"
              v-for="(file, index) of request.req_files"
              :key="index"
              download
              :address="
                request.id
                  ? learning_src +
                    ...
                    $route.params.id +
                    '/file_delete/'
                  : kernel_src + 'draft/delete/' + file.file_id
              "
              :downloadAddress="
                request.id
                  ? null
                  : kernel_src + 'draft/download/' + file.file_id
              "
              request
              hideAuthor
            />
...
export default {
  ...
  created() {
    ...
    if (this.user.isPlAdmin) {
      this.$axios
        .get(this.learning_src + "user/?filter=project-manager")
        .then((data) => {
          this.managers = data.data.map(
            (manager) =>
              new Object({
          ...
          this.request.manager =
            this.user.id ||
            (this.managers.length ? this.managers[0].id : this.user.id);
        });
    } else {
    ...
    if (this.$route.params && this.$route.params.id) {
      ...
      this.$axios
        .get(this.learning_src + "request/" + this.$route.params.id + "/short/")
        .then((data) => {
  ...
  beforeRouteLeave(to, from, next) {
    if (this.formEdited && !this.formSaved) {
      this.$bvModal
        .msgBoxConfirm(
          "Хотите сохранить черновик и продолжить заполнение позже?",
          {
            buttonSize: "md",
  ...
  methods: {
    ...
    sendRequest(type, next) {
      ...
      if (type === "draft" || !this.$v.$invalid) {
        ...
        if (type !== "draft") {
          ...
          this.$bvModal
            .msgBoxConfirm(
              "Перед отправкой проверьте, правильно ли заполнена заявка. Отменить действие будет нельзя.",
              {
    ...
    removeRequest() {
      this.$store
        .dispatch("project/removeRequest", { id: this.$route.params.id })
        .then((deleted) => {
    ...
    checkTemporyDraft() {
      ...
      this.$axios.post(this.kernel_src + "draft/", params).then((data) => {
        if (data.data && data.data.state) {
          ...
          this.$bvModal
            .msgBoxConfirm("Восстановить последние несохраненные данные?", {
    ...
    saveTemporyDraft() {
      ...
      this.$axios
        .post(this.kernel_src + "draft/save/", sendDraft)
        .then((data) => {
          this.draft.updated = data.data.date;
        });
    },
    savePDF() {
      const managerName = this.managers.find(
        (manager) => manager.id === this.request.manager
      );

      const docDefinition = {
        info: {
          title:
            "Заявка на проект " + this.request.name ? this.request.name : "",
        ...
        content: [
            ...
            text: [
              {
                text: [
                  ...
                  {
                    text:
                      this.formatDate(this.semesterActual.start_date) + " г.",
                    bold: true,
                  },
                ],
              },
          ...
          {
            ul: this.request.programs.map((program) =>
              Object({
                text:
                  this.getProgram(program).uid +
                  "  " +
                  this.getProgram(program).name,
                color: "#555555",
              })
            ),
          },
          ...
          {
            ul: this.request.req_files.map((file) =>
              Object({
                text: file.human_name,
                color: "#555555",
              })
            ),
          },
      ...
      pdfMake
        ...
        .download(
          "Проект. " + (this.request.name ? this.request.name : "") + ".pdf"
        );
    },
  },
  computed: {
    ...
    ...mapGetters("api", ["semesterActual"]),
    computedForm() {
      return JSON.stringify(this.request);
    },
	maxInputesLabel: () => window.innerWidth < 576 ? 'Макс. количество экземпляров проекта' : 'Максимальное количество экземпляров проекта'
  },
  watch: {
...
@media (max-width: 575px) {
	...
	/deep/ .form-group-amount {
		max-width: 180px;
	}

	/deep/ .form-group-amount .col-form-label {
		font-size: 13px;
		line-height: 16px;
	}
